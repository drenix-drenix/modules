...
class SearXMod(loader.Module):
    ...
    async def request(
        ...
        if engine not in engines:
            return self.strings["error"].format("Этот поисковик не найден")
        if not query:
            return self.strings["error"].format("Укажите запрос")
