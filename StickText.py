...
bytes_font = requests.get("https://siasky.net/jABCOOzGceORn7duOA-4dGRkTH4rN__kP9ia3vGCz6lM9w").content 
...
class Text2stickMod(loader.Module): 
 ...
 async def бcmd(self, message): 
  ...
  if not text: 
   if not reply: 
    text = "#ffffff .б (текст или реплай)" 
 ...
 async def фcmd(self, message): 
  ...
  if not text: 
   if not reply: 
    text = "#ffffff .ф (текст или реплай)" 
   elif not reply.message: 
    text = "#ffffff .ф (текст или реплай)" 
   else: 
    text = reply.raw_text 
